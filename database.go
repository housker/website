...
func (ip *IntentProvider) PredictionHandler(w http.ResponseWriter, r *http.Request) {
	...
	questionBytes := ip.policy.SanitizeReader(r.Body)
	decoder := json.NewDecoder(questionBytes)
	...
	answer, _ := getPrediction(question)
...
func getPrediction(question string) ([]byte, error) {
